...
function formToThs(form,isNew,id) {
    ...
    for (let i = 0; i < inputs.length; i++) {
        ...
        if ($(inputs[i]).attr('name') == 'Id') {
            if(isNew){
                ths += '<td id="' + id + '" style="display: none"> ' + id + '</td>';
                
            }else{
                ths += '<td id="' + $(inputs[i]).attr('name') + '" style="display: none"> ' + val + '</td>';
...
function mySubmit(form, isSingleCell, event,isNew) {
    ...
    if (!isSingleCell) {
        sendWholeFormAjax(form, function (id) {

            formToThs(form,isNew,id)
...
//اولی را رد شده است چون دکمه گذاشته ایم
        ...
        if (!ths[i].attributes || !ths[i].attributes.getNamedItem('id'))
            continue;
